...
export default function SubtopicLearn() {
  ...
  useEffect(() => {
    if (!step || !pathData) return;
    const module = Array.isArray(pathData.modules)
      ...
      : null;
    const resolved =
      module?.steps?.find((item) => item.id === step.id) || null;
    if (resolved) {
      setContent({
  ...
  return (
    <main className="container">
      <header className="header">
        ...
        <Link className="ghost" href={`/learn/${params?.slug || ""}`}>
          Back
        </Link>
      </header>
      ...
      {!plan || !step ? (
        <section className="panel">
          ...
          <Link className="primary" href={`/learn/${params?.slug || ""}`}>
            Back to topic
          </Link>
        </section>
      ) : (
